...
public class DiscordCommandHandler extends ListenerAdapter {
    ...
    public DiscordCommandHandler(JustSyncApplication integration) {
        ...
        integration.getGuild()
            ...
            .addCommands(
                ...
                Commands.slash(
                    "unlink", "Unlink your discord account from every linked minecraft account"),
                Commands.slash("list", "List the currently online players"),

                // restricted
                Commands.slash("justsync", "Misc linking stuff")
                    .addSubcommands(new SubcommandData("get", "Retrieve linking information")
                                        .addOption(OptionType.USER, "user", "whose data to get")
                                        .addOption(OptionType.STRING, "mc-name",
                                            "minecraft username to get data"),
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        if (event.getMember() == null
            || event.getMember().getGuild() != this.integration.getGuild()) {
            return;
        }
        switch (event.getName()) {
            case "link" -> {
                String code = event.getOptions().getFirst().getAsString();
                String message = this.integration.getLinkManager().confirmLink(
                    event.getUser().getIdLong(), code);
                event.reply(message).setEphemeral(true).queue();
            }

            case "unlink" -> {
                Optional<PlayerLink> dataOptional =
                    this.integration.getLinkManager().getDataOf(event.getMember().getIdLong());
    ...
    private void linkingCommand(SlashCommandInteractionEvent event) {
        ...
        if (!isSelf
            && !PermissionUtil.checkPermission(event.getMember(), Permission.MODERATE_MEMBERS)) {
            event.reply("Insufficient permissions").setEphemeral(true).queue();
            return;
        }
    ...
    private void listAlts(SlashCommandInteractionEvent event) {
        List<PlayerLink> linksWithAlts = this.integration.getLinkManager()
                                             .getAllLinks()
                                             .filter(link -> link.altCount() > 0)
                                             .toList();
    ...
    private void linkingWithMember(SlashCommandInteractionEvent event, Member target) {
        Optional<PlayerLink> dataOptional =
            this.integration.getLinkManager().getDataOf(target.getIdLong());
        if (dataOptional.isEmpty()) {
            event.reply(target.getAsMention() + " is not linked to any player")
                .setEphemeral(true)
                .queue();
            return;
        }
        ...
        if (Objects.equals(event.getSubcommandName(), "get")) {
            ...
            if (data.altCount() != 0) {
                text += "\nwith " + data.altCount() + " alts: "
                    + String.join(", ", data.getAlts().stream().map(PlayerData::getName).toList());
            }
    ...
    private void linkingWithPlayer(String subCommand, InteractionHook hook, GameProfile profile) {
        ...
        if (Objects.equals(subCommand, "get")) {
            ...
            if (member.isPresent()) {
                hook.editOriginal(
                        profile.getName() + " is linked to " + member.get().getAsMention())
                    .queue();
            } else {
